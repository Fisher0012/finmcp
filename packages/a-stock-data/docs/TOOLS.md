...
全市场融资融券余额序列（沪深合计, 日度）。**返回**: 按日期的融资余额/融券余额/合计（亿元）。

## 33. `get_holder_number`

个股股东户数趋势（多期）。**返回**: 各报告期股东户数与环比变化。

## 34. `get_top_float_holders`

十大流通股东（最新期）。**返回**: 股东名称/持股数/占流通比/较上期变动。

## 35. `get_block_trades`

大宗交易记录（近区间）。**返回**: 成交日期/价格/金额/买卖营业部。

## 36. `get_share_unlock`

限售解禁日历（个股）。**返回**: 解禁日期/解禁股数/占总股本比/股份类型。

## 37. `get_consensus_forecast`

券商一致预期（近半年盈利预测按年度聚合）。**返回**: 各年度 EPS 中位数/净利中位数(亿)/机构数、评级分布。同机构同季度去重。

## 38. `get_meso_indicator`

中观行业指标（10 类: 车/猪/大宗/硅锂现货/BDI/用电量/物流/70城房价/费半/企业商品价格）。**返回**: 指标序列（期数按指标频率）。

## 39. `get_global_context`

外部市场环境（美三大指数/NVDA/汇率/美元指数/美债收益率）。**返回**: 各标的最新值与近期变动。
